...
- 編輯分鏡旁白時，即時顯示字數、預估旁白秒數，以及單分鏡上限提示
- 若旁白超過單分鏡上限，前端會直接阻止送出修改

![image](./image/screen2.png)

- 支援以中文或英文產生分鏡 prompt，且預設為中文
- 生成分鏡 prompt 時，會先避免輸出可能涉及 OpenAI Sora 2 模型規範的高風險描述，例如真實品牌名稱、清楚 logo、商標或可辨識招牌
- 重新產生單一分鏡前，會提示該內容可能被系統自動拆分成多個分鏡
...
媒體後端預設為 `ffmpeg`。只有當你明確把 `.env` 內的 `MEDIA_BACKEND` 改成 `gstreamer` 時，系統才會切換到 GStreamer。
...
4. 檢查並編輯系統生成的旁白與影片提示詞。
	編輯旁白時，畫面會顯示目前字數、預估旁白時長，以及建議字數上限。
	若超過單分鏡上限，前端會停用「儲存修改」並阻止送出。
5. 執行分鏡生成，產生各分鏡的音訊與影片片段。
6. 視需要重生單一分鏡。
	重新產生前，系統會提醒該內容可能被自動拆分成多個連續分鏡。
7. 手動執行影片串接，輸出最終影片。
...
- 專案分鏡總數上限預設為 `300`。
- 單一分鏡的有效旁白時長上限預設為 `12` 秒；這個限制會同時受到 Sora 2 支援片段長度約束。
...
- Web UI 在儲存分鏡修改前，也會先根據字數提供預估秒數與上限提示，避免使用者在重生前才發現內容過長。
...
3. 把摘要拆成分鏡級別的旁白與影片提示詞。
	這個階段會優先避開可能觸發 OpenAI Sora 2 規範的 prompt 描述，例如真實品牌名稱、清楚 logo、商標、招牌或其他可辨識品牌識別，並改寫成泛化場景。
4. 在 Web UI 檢查並編輯提示詞。
...
這個專案使用 `.env` 的 `MEDIA_BACKEND` 來切換媒體後端。

- 若沒有特別設定，或設定為 `MEDIA_BACKEND=ffmpeg`，就會使用預設的 ffmpeg。
- 若要切換到 GStreamer，請把 `MEDIA_BACKEND` 改成 `gstreamer`。
...
如果之後要切回 ffmpeg，只要把 `.env` 改回：

```env
MEDIA_BACKEND=ffmpeg
```

修改 `.env` 後，重新啟動應用程式就會生效。
